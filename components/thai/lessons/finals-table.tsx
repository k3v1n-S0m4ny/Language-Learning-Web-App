  ...
  return (
    <div className="flex flex-col gap-3">
      {items.map((item) => (
        <div
          key={item.id}
          className="flex flex-col gap-1 rounded-xl border border-border-base bg-surface p-4 sm:flex-row sm:items-center sm:gap-4"
        >
          <div className="w-16 shrink-0 font-mono text-lg font-semibold text-foreground">
            [{item.initialIpa}]
          </div>
          <div className="font-thai shrink-0 text-lg text-foreground-muted">
            {item.metadata.letters.join(" ")}
          </div>
          <div className="flex items-baseline gap-2 text-sm">
            <span className="font-thai text-lg text-foreground">
