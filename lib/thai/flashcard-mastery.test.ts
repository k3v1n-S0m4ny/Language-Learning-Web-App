...
// Minimal synthetic content: one unit-2 consonant (the flashcard pilot) and
// one unit-3 consonant (unchanged MCQ), both with a final sound so they are
...
const ITEMS: ReachabilityItem[] = [
  ...
  { id: "consonant:u3", kind: "consonant", unit: 3, drillable: true, finalIpa: "t", display: "จ", metadata: {} },
];
...
test("units 3-5 keep the original letter-sound/letter-class/audio-letter MCQ trio", () => {
  const map = reachableDrillTypesForUnit(3, ITEMS);
  assert.deepEqual(map.get("consonant:u3"), ["letter-sound", "letter-class", "audio-letter"]);
});
