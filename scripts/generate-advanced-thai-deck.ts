 ...
 * glosses, the English context label, and the vocabulary extraction.
 *
 * A third pass extracted GRAMMAR patterns, with a cross-theme dedupe so a new
 * theme could not re-teach a rule an earlier one already carried. Both are gone:
 * the ladder redesign deleted the grammar card kind, and with no card to write to
 * there is nothing for the pass to produce.
 *
...
import type { PhraseEntry, Theme, VocabEntry } from "../seed/advanced-thai/types";
...
// which is why the seed script's pos/register assertions should never fire on
...
async function main() {
  ...
  if (DRY_RUN) {
    ...
    const requests = phraseRequests + 1; // + vocab
    ...
    const estOut = clauses.length * 90 + MAX_VOCAB * 120;
  ...
  // vocab carries whatever an earlier run already produced.
  ...
  console.log(`\nWrote ${OUT}`);
  console.log(`  ${phrases.length} phrases, ${vocab.length} vocab`);
  const cost = PRICE
